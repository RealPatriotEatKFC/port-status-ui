...
export default function EquipmentList() {
  ...
  const handleDeleteEquipment = (e: React.MouseEvent, equipmentId: string) => {
    ...
    if (confirm(t.equipment.deleteConfirm)) {
  ...
  return (
    <div className="w-80 bg-gray-50 dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col h-screen">
      ...
      <div className="p-4 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-2">{t.equipment.title}</h1>
        <button
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {filteredEquipments.length === 0 ? (
          <div className="text-center text-gray-500 dark:text-gray-400 py-8">
            <p className="mb-2">{t.equipment.noEquipment}</p>
            <p className="text-sm">{t.equipment.addEquipmentPrompt}</p>
          </div>
        ) : (
          filteredEquipments.map((equipment) => (
            ...
            >
              ...
              <div className="space-y-1 text-sm text-gray-600 dark:text-gray-300">
                ...
                <p>
                  <span className="font-medium">{t.common.ports}:</span> {equipment.layout.rows}{t.common.rows} × {equipment.layout.cols}{t.common.cols}
                </p>
